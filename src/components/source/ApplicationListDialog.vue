    ...
    <el-table :data="tableData" border stripe style="width: 100%">
      ...
      <el-table-column label="操作" width="160">
        <template #default="scope">
          <el-button type="success" size="small" @click="handleApprove(scope.row)" :disabled="scope.row.sourceAgreed === true">同意</el-button>
          <el-button type="danger" size="small" @click="handleReject(scope.row)" :disabled="scope.row.sourceAgreed === false" style="margin-left: 8px;">拒绝</el-button>
        </template>
      </el-table-column>
      <el-table-column label="申请状态" min-width="160">
        <template #default="scope">
...
function handleApprove(row) {
  axios.post(`http://localhost:8080/api/applications/source/approve/${row.id}`, null, { withCredentials: true })
    .then(res => {
      if (res.data && res.data.code === 1) {
        row.sourceAgreed = true;
        ElMessage.success(`数源方同意ID为${row.id}的申请`);
      } else {
...
function handleReject(row) {
  axios.post(`http://localhost:8080/api/applications/source/reject/${row.id}`, null, { withCredentials: true })
    .then(res => {
      if (res.data && res.data.code === 1) {
...
function getStatusText(row) {
  if (row.sourceAgreed === true) return '数源方同意,等待治理方处理'
  return '数源方拒绝或未处理'
}
