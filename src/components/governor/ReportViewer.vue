...
const printReport = () => {
  ...
  if (!printWindow) {
    ElMessage.warning('浏览器已阻止弹出窗口，请允许弹出窗口后重试')
    return
  }
  ...
  printWindow.document.write(`
    <html>
      ...
      <body>
        <h1>审查报告 ${props.objectId ? `(对象ID: ${props.objectId})` : ''}</h1>
        <hr>
        ${formattedReportContent.value}
        <hr>
        <div style="text-align: right; color: #909399; margin-top: 30px;">
